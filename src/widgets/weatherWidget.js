...
export class WeatherWidget extends BaseWidget {
    ...
    _render(d) {
        ...
        topRow.add_child(new St.Label({ text: d.icon, style_class: 'tahoe-label-medium' }));
        ...
        infoCol.add_child(new St.Label({ text: `💧 ${d.humidity}  💨 ${d.wind}`, style_class: 'tahoe-label-small tahoe-muted' }));
        ...
        this._content.add_child(new St.Widget({ style: 'height:1px; background:rgba(255,255,255,0.12); margin:4px 0;', x_expand: true }));
        ...
        d.forecast.slice(0,6).forEach(f => {
            ...
            cell.add_child(new St.Label({ text: f.icon, style_class: 'tahoe-forecast-icon', x_align: Clutter.ActorAlign.CENTER }));
    ...
    _renderError(msg) {
        ...
        box.add_child(new St.Label({ text: '☁️', style_class: 'tahoe-label-large', x_align: Clutter.ActorAlign.CENTER }));
